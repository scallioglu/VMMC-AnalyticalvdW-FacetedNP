...
void PolyClip::PloygonOpration::DetectIntersection(Polygon& clip_poly, Polygon& sub_poly, bool &error_flag)
{
	...
	for (auto iter1 = clip_poly.begin(); loop1_count<loop1_total; ++loop1_count)
	{
        ...
        error_flag = false; //initialization
        if (intersection_point_count % 2 == 1) {
           error_flag = true;
        }
        //std::cout << "error_flag_PolygonClipping " << error_flag << std::endl;
