...
double LJPotentialBetweenTwoCubes(double Sigma, double Epsilon, double LJCutOff,
    ...
    int row1 = size1 * size1 * (size1+1) * aspectratio / 2.0;
    int row2 = size2 * size2 * (size2+1) * aspectratio / 2.0;
...
void findVerticesOfCube(double CubeSideLength, double Height, double AASigma, double COMX, double COMY, double COMZ, const Vector3 VectorX, const Vector3 VectorY, const Vector3 VectorZ, double Vertex[][3]){
...
void Reorientation(int i, int j, double Cube2SideLength, double Cube2Height, double AASigma, double BoxLength,
    ...
    */
    ...
    findVerticesOfCube(Cube2SideLength,Cube2Height,AASigma,RotatedParticle2Centroid[0],RotatedParticle2Centroid[1],RotatedParticle2Centroid[2],RotatedParticle2VectorsX,RotatedParticle2VectorsY,RotatedParticle2VectorsZ,CubeVertex);
...
double calc_ver(double Particle2VerticesEdge[][3],double AASigma, double catr1, double atre1, double catr2, double atre2, double crep, double repe, double Epsilon, double lc, double aspectratio, vector<double>& y_coord11, vector<double>& z_coord11, double ds, bool error_flag) {
...
bool CheckOverlapForTwoCubes(const int i, const int j, double BoxLength, double AASigma, double CubeSideLength1, double CubeSideLength2,
    ...
    findVerticesOfCube(CubeSideLength1,CubeSideHeight1,AASigma,ParticleC1[0],ParticleC1[1],ParticleC1[2],VectorX_1,VectorY_1,VectorZ_1,cube1initial);
    double cube2initial[6][3];
    findVerticesOfCube(CubeSideLength2,CubeSideHeight2,AASigma,ParticleC2[0],ParticleC2[1],ParticleC2[2],VectorX_2,VectorY_2,VectorZ_2,cube2initial);
    ...
    double ds;
    double lc = Size1/2.0;
    Reorientation(i, j, CubeSideLength2, CubeSideHeight2, AASigma, BoxLength, ParticleC1, ParticleC2, VectorX_1, VectorY_1, VectorZ_1, VectorX_2, VectorY_2, VectorZ_2, cube2Vertex, y_coord11, z_coord11, ds, lc);
...
void VolumeMove(int NumberOfParticles, double BoxLength, double AASigma, double aspectratio, double volrate,
    ...
    double FirstPositionX = Rx[0] * volrate / BoxLength;
    double FirstPositionY = Ry[0] * volrate / BoxLength;
    double FirstPositionZ = Rz[0] * volrate / BoxLength;
    double FirstCubeAlpha = 0.0;
    double FirstCubeBeta = 0.0;
    double FirstCubeGamma = 0.0;
...
pair<double, bool> EnergyBetweenTwoParticles(string model, double AASigma, double CGSigma, double CGEpsilon, double cutoffCG, double Hamaker, double AtomDensity, int Size_i, int Size_j, double aspectratio, double BoxLength, const Vector3 ParticleiCentroid, const Vector3 ParticlejCentroid, const Vector3 VectorX_i, const Vector3 VectorY_i, const Vector3 VectorZ_i, const Vector3 VectorX_j, const Vector3 VectorY_j, const Vector3 VectorZ_j) {
    ...
    if (model == "AACG") {
        ...
        Reorientation(i,j,Cube2SideLength,Cube2Height,AASigma,BoxLength,Particle1Centroid,Particle2Centroid,VectorX_1,VectorY_1,VectorZ_1,VectorX_2,VectorY_2,VectorZ_2,Particle2VerticesEdge,y_coord11,z_coord11,ds,lc);
...
pair<double, bool> OneParticleEnergy(string model, int i, double Size_i, double aspectratio, double cutoffCG, double Hamaker, double AtomDensity, const double Rx_i, const double Ry_i, const double Rz_i, 
                         ...
                         int NumberOfParticles, double BoxLength, double CGEpsilon, double CGSigma, double AASigma, double globalminEnergy, vector<int>& clusterParticleList){
   ...
   for (int j = 0; j < NumberOfParticles; ++j) {
       if (j != i && find(clusterParticleList.begin(), clusterParticleList.end(), j) == clusterParticleList.end()) {
          ...
          double CutOff =  2.5 * MaxSize * CGSigma * max(1.0,aspectratio);
...
void writeTrajectory(int TrajectoryInterval, string& Style, int Step, int NumberOfParticles, double AASigma, double CGCubeSize, double aspectratio, double CGSigma, double BoxLengthHalf,
    ...
    if (Step % TrajectoryInterval == 0) {
        ...
        if (Style == "AA") {
            int numAtoms = CGCubeSize * CGCubeSize * (CGCubeSize + 1) * aspectratio * NumberOfParticles / 2.0;
            ...
            double COMX,COMY,COMZ;
            int NumAtomsPerCube = pow(CGCubeSize, 2)*aspectratio*(CGCubeSize+1)/2.0;
            ...
            for (int i = 0; i < NumberOfParticles; ++i) {
                ...
                COMY = Ry[i];
                COMZ = Rz[i];
                findVerticesOfCube(CubeSideLength, CubeHeight, AASigma, COMX, COMY, COMZ, VectorX[i], VectorY[i], VectorZ[i], Vertex);
                for (int j = 0; j < 6; ++j) {
...
int main() {
    string Style = "AA"; //  AA, Virtual, Vertex
    string model = "vdW"; //AACG, vdW

    int NumberOfParticles = 432;
    double BoxLength = 200.912;
    double BoxLengthHalf = BoxLength/2.0;
    ...
    double globalminEnergy = kBT; // Global minimum energy between two particles (parallel configuration)
    ...
    double MaxMove = AASigma  * 4; // maximum allowed translation for a particle in each trial step
    ...
    int ProductionSteps = 65000000; // Production time steps
    int EnergyOutputInterval = 1000; // Energy output frequency
    ...
    double aspectratio = 1.0; // height / length of particle
    double oldvolfrac = NumberOfParticles*pow(CubeSideAASize*AASigma,2)*(CubeSideAASize+1)*0.5*AASigma*aspectratio/pow(BoxLength,3);
    unordered_map<double, double> SizeRatioDictionary = {{CubeSideAASize, 1.0}};
    double CGSigma, CGCubeSize, cutoffCG, CGEpsilon, Hamaker, AtomDensity;
    ...
    for (int Step = 1 + restartStep; Step <= restartStep + EquilibrationSteps + ProductionSteps; ++Step) {
        ...
        double singleOrCluster = 0.0; //single moves only for this weak interaction case
        if (singleOrCluster < SingleMoveProbability) {
           ...
           MaxClusterSize = NumberOfParticles / 6.0; // After 50 particles, it is time consuming to create clusters and then reject
        ...
        if (EarlyTermination == false) {
           ...
           if (isTranslation == true && MaxClusterSize > 1) { // damping coefficient calculation for clusters
              ...
              D_value = Dt;
           } else if (isRotation == true && MaxClusterSize > 1) {
        ...
        if (Step % volumeMoveInterval == 0 || Step == 10000) { //first compression step is very early as it starts from 100% volume fraction (no need to run long equilibration for volume fraction=1 case, as there is no space to move)
